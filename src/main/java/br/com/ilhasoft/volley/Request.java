...
class Request extends StringRequest {
    ...
    void setConntentType(String contentType) {
...
//        return Response.success(parsed, HttpHeaderParser.parseIgnoreCacheHeaders(response));
    ...
    private Cache.Entry fakeCache(NetworkResponse response){
        ...
        if (invalidCacheInfoOnResonse(cache)) {
            int fakeCacheTime;
    ...
    private static boolean invalidCacheInfoOnResonse(Cache.Entry mFakeCache) {
    ...
    public boolean shouldCache() {
        try {
            return mShouldCache;
//            return this.getClass().getDeclaredField("mShouldCache").getBoolean(this);
        }catch (Exception e){
            e.printStackTrace();
        }
